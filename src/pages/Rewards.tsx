...
import { UploadCloud, CheckCircle2, Leaf, Clock, XCircle, Gift, Tag } from 'lucide-react';
...
export default function Rewards() {
  ...
  return (
    <div className="fade-in bottom-nav-safe">
      ...
      {/* Compact dark hero */}
      <div className="glass-panel-dark mx-4 mt-4 p-5 relative overflow-hidden rounded-2xl mb-5">
        <div className="absolute inset-0 opacity-5 bg-[url('https://www.transparenttextures.com/patterns/leaves.png')]" />
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-1">
            <GCoinIcon size={38} />
            <h1 className="text-xl font-black text-white" style={{ fontFamily: 'Outfit,sans-serif' }}>G Coins</h1>
          </div>
          <p className="text-[rgba(200,230,201,0.8)] text-xs mb-4 leading-relaxed">
      ...
      {/* Tabs */}
      <div className="flex mx-4 mb-5 bg-white/60 rounded-xl p-1 border border-[rgba(46,125,50,0.12)]">
        <button
          onClick={() => setActiveTab('earn')}
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg font-bold text-sm transition-all ${activeTab === 'earn' ? 'bg-[#2e7d32] text-white shadow-sm' : 'text-[#5f7a60]'}`}
        >
          <UploadCloud className="w-4 h-4" /> Earn
        </button>
        <button
          onClick={() => setActiveTab('redeem')}
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg font-bold text-sm transition-all ${activeTab === 'redeem' ? 'bg-[#ffb300] text-black shadow-sm' : 'text-[#5f7a60]'}`}
        >
      ...
      {/* EARN TAB */}
      {activeTab === 'earn' && (
        <div className="px-4 space-y-5 fade-in">
          {/* Upload Card */}
      ...
      {/* REDEEM TAB */}
      {activeTab === 'redeem' && (
        <div className="px-4 fade-in space-y-6">
          {/* Voucher Store */}
          <div>
